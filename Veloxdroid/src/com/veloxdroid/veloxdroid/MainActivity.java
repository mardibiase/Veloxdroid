...
public class MainActivity extends Activity {
	...
	protected void onStart() {
		super.onStart();
		// check wheter it's the first time you use the app
		// if the user is logged in, checkLogin() returns true
		// else returns false
		if (checkLogin()) {
			// TODO - Check if the user has selected to automatically download
			// autovelox files

			// automatically fires download file - we do not pass from
			// downloadFiles
			// but instead we directly instantiate a new DownloadTask and
			// execute it
	...
	// method that downloads the file of the autoveloxes
	// fires when clicking on the button Download on the  GUI
	public void downloadFiles(View view) {
		Toast toast = Toast.makeText(getApplicationContext(), "Download file",
				5);
		toast.show();
		// execute this when the downloader must be fired
		// we have to do in an AsyncTask separately because Android doesn't
		// allow
		// to do this in a standard activity
